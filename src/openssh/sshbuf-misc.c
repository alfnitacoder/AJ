...
// Stub out sshbuf_dump_data - fprintf not available in kernel
void
sshbuf_dump_data(const void *s, size_t len, FILE *f)
{
	// Stub - file I/O not available in kernel
	(void)s; (void)len; (void)f;
}

// Stub out sshbuf_dump - fprintf not available in kernel
void
...
// Stub out base64 functions - not available in kernel
int
sshbuf_dtob64(const struct sshbuf *d, struct sshbuf *b64, int wrap)
{
	// Stub - base64 encoding not available in kernel
	(void)d; (void)b64; (void)wrap;
...
{
	// Stub - base64 decoding not available in kernel
	(void)buf; (void)b64;
	return SSH_ERR_INVALID_ARGUMENT;
}
